...
string* ps = new string("C++");  // ps指向的对象的内容为 "C++"
...
if(p1 && p1->empty()) // 指针p1非空且其指向的对象为非空string
    *p1 = "Lisha"; // *p1为解引用
\end{lstlisting}\vspace{-1.5mm}
\end{blueblock}
...
\texttt{unique\_ptr}独自拥有所指向的动态对象，也就是说只能有一个\texttt{unique\_ptr}指向给定
的对象：
...
~\\
\uncover<2->{如果类的数据成员\alert{含有动态对象}，使用这些\alert{默认}成员函数会有什么问题？}
...
\column{0.3\textwidth}
\begin{yellowblock}<3->{说明}
\texttt{a2}和\texttt{a1}的数据成员\texttt{m\_array}指向同一个动态数组对象，若\texttt{a1}先被销毁，该动态内存被释放，则\texttt{a2}被销毁时该动态内存无法被释放
\end{yellowblock}
\begin{redblock}<4->{注意}
此外从实际含义上，复制\texttt{a1}到\texttt{a2}时需要复制\alert{动态对象的内容}，而不是地址
\end{redblock}
...
$\bullet$~重载的\texttt{MyStr}类运算符\texttt{+}，将两个形参\texttt{MyStr}类对象中的字符连接起来，形成一个新的\texttt{MyStr}类对象\texttt{res}，并以值的形式返回~\alert{\texttt{res}的拷贝}
\end{yellowblock}
...
\end{yellowblock}
\begin{redblock}<3->{注意}
赋值运算符函数中的\texttt{if}语句是必需的，缺少此判断语句可能会出现严重的内存错误
\end{redblock}
...
\end{lstlisting}
\end{blueblock}
\begin{blueblock}{\texttt{MyStr}类\texttt{operator+}运算符声明}
\begin{lstlisting}[moreemph={MyStr}]
...
$\bullet$~右值不可写的特性和\texttt{const}对象是一致的，所以\texttt{MyStr}的复制构造函数可以接受右值对象。
\end{greenblock}

\begin{greenblock}<3->{问题}
\alert{\texttt{s3}~新开辟了动态内存空间}，复制临时对象申请的动态内存中的内容，而\alert{临时对象申请的动态内存又被释放}。对此有什么想法？
\end{greenblock}
...
给参数为临时对象的构造函数定义更\alert{精准匹配}的\alert{参数为右值引用}的移动构造：
...
\column{0.27\textwidth}
\begin{yellowblock}<2->{说明}
相比复制构造函数，\alert{没有分配新的动态内存}，也\alert{没有对动态内存数据的复制}。
\end{yellowblock}
\begin{greenblock}<3->{问题}
...
$\bullet$~否则，将尾节点指向该结点，并将尾指针后移，使其指向新的尾结点
\end{yellowblock}
...
\column{0.27\textwidth}
\begin{yellowblock}{说明}
$\bullet$~树是一种很自然的\alert{递归}结构，因此\texttt{insert}和析构函数都分别调用私有成员函数\texttt{insert\_}和\texttt{destory}，分别递归进行插入和销毁子树操作\\
$\bullet$~\texttt{inorder}函数执行\alert{中序遍历}操作，也是递归操作，其第二个参数为遍历时对元素进行操作的函数\\
$\bullet$~\texttt{search}函数调用\texttt{search\_}，从\alert{根节点}开始进行\alert{二分搜索}
\end{yellowblock}
...
\column{0.65\textwidth}
\begin{block}{二叉树的遍历}
$\bullet$~根据某种次序访问树中每个结点一次且仅一次。\\
$\bullet$~访问的过程中可以根据需要对结点的数据进行不同的处理操作，但\alert{不能}改变原来的结构。
\end{block}
...
\alert{\uncover<22->{中序遍历二叉搜索将得到有序序列}}
\end{block}
\begin{block}<23->{后序遍历}
