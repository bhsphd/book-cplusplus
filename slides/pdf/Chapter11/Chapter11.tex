...
\begin{frame}[fragile]{11.2~容器\small{——容器概述}}
	...
	\begin{columns}
		...
		\begin{blueblock}{vector的插入和删除数据操作}
			\begin{lstlisting}[basicstyle=\small\ttfamily]
vector<int> vec = {0, 1, 2, 3, 4};
vec.insert(vec.begin(), 10); //在第一个位置插入整数10
...
\begin{frame}[fragile]{11.2~容器\small{——关联容器}}
	\begin{block}{关联容器}
		不同于顺序容器，关联容器采用\alert{非线性}结构。通常情况下，关联容器是通过\alert{树结构}实现的，其元素通过\alert{关键字}来访问。STL中主要的两个关联容器是：
		\begin{itemize}
			\item \textbf{set}
			\item \textbf{map}
		\end{itemize}
...
...
		...
		\only<1>{\begin{yellowblock}{说明}
				...
				$\bullet$ 执行第四行代码时，如找到关键字为number的元素则其值自增，否则以此关键字生成新的元素\\
			\end{yellowblock}}
		\only<2->{\begin{redblock}{注意}
...
\begin{frame}[fragile]{11.2~容器\small{——高效使用容器}}
	\begin{yellowblock}{6.正确使用map的insert和下标运算符}
		...
		\end{itemize}
	\end{yellowblock}
	如果你需要修改已有元素的值且不在意下标运算符是否会插入新的元素，则可以尽情享用下标运算符带来的方便。比如：
	\begin{columns}[T]
		\column{0.65\textwidth}
...
for_each(vi.begin(), vi.end(), uf);
		...
		\begin{yellowblock}
			...
			$\bullet$ 调用uf的时候，它会将参数\_1和参数n传递给filter函数并调用之来完成实际的工作\\
