...
\begin{frame}[fragile]{迭代器\small{——实现Find函数模板}}
...
\subsection{使用迭代器}
\begin{frame}[fragile]{迭代器\small{——使用迭代器}}
...
\begin{frame}[fragile]{容器\small{——容器概述}}
...
\subsection{顺序容器}
\begin{frame}[fragile]{容器\small{——顺序容器}}
    \begin{block}{顺序容器}
...
\subsection{关联容器}
\begin{frame}[fragile]{容器\small{——关联容器}}
    \begin{block}{关联容器}
...
\subsection{高效使用容器}
\begin{frame}[fragile]{容器\small{——高效使用容器}}
...
\subsection{算法概述}
\begin{frame}[fragile]{泛型算法\small{——算法概述}}
    \begin{block}{标准库算法}
...
\subsection{向算法传递函数}
\begin{frame}[fragile]{泛型算法\small{——向算法传递函数}}
...
\subsection{参数绑定}
\begin{frame}[fragile]{泛型算法\small{——参数绑定}}
...
\begin{frame}[fragile]{泛型算法——\small{使用function}}
...
bind(Less, std::placeholders::_2, std::placeholders::_1);
        ...
        \begin{yellowblock}
            {说明}
            定义了\texttt{bool(LargeData*,LargeData*)}调用形式的CallType类型\\
        \end{yellowblock}
    \end{columns}
    \begin{columns}[T]
        \column{0.65\textwidth}
...
if (f4(&a, &b)) {/*...*/}
        ...
        \begin{yellowblock}
            {说明}
            使用新创建的function对象对不同的调用对象使用形式进行统一\\
        \end{yellowblock}
    \end{columns}
\end{frame}
