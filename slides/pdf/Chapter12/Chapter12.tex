...
\begin{frame}[fragile]{12.6~union类型}
    \begin{block}
        {union类型}
        一种特殊的复合数据类型，可以实现一个对象用作多种数据类型使用的作用，例如一个对象既可以用来存储整型数据，又可以用来存储浮点型数据。union类型可以包含多个数据成员，但这些数据成员共享同一段内存空间，空间大小取决于内存占用最大的数据成员类型。使用时只会有一个数据成员处于定义后的激活状态，其他数据成员则处于未定义状态。
    \end{block}
\end{frame}
