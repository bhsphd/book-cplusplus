...
double Calculator::doIt(const string & exp) {
    for (auto it = exp.begin(); it != exp.end();) {
        if (isNum(it))
            ...
            auto oo=Factory::create(*it++); //产生一个当前运算符类对象
...
\begin{frame}[fragile]{运行时类型识别\small{——dynamic\_cast运算符}}
...
\end{lstlisting}
        ...
        \begin{yellowblock}
            {说明}
            下方代码：\\
            $\bullet$ 第一个转换中b1与基类对象绑定，转换失败\\
            $\bullet$ 第二个转换中b2与派生类对象绑定，转换成功，执行name调用\\
        \end{yellowblock}
        \begin{redblock}
            {建议}
...
\begin{frame}[fragile]{运行时类型识别\small{——typeid运算符}}
...
title1 48.99 2017
    \end{lstlisting}
        ...
        \begin{yellowblock}
            ...
            $\bullet$ 由于tie函数创建的tuple包含4个对象的引用，所以有最终的输出结果\\
        \end{yellowblock}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]{12.7~标准库特殊工具\small{——日期和时间}}
    C++提供chrono库对时间和日期进行操作，其包含三种时钟类：
    \begin{itemize}
        \item \texttt{system\_clock}
        \item \texttt{steady\_clock}
        \item \texttt{high\_resolution\_clock}
    \end{itemize}
    其中system\_clock和high\_resolution\_clock是实时时钟（real time clock），会随如夏令时的时间调整而改变，而steady\_clock是单调时钟（monotonic clock），不随外界时间调整而改变。
\end{frame}
